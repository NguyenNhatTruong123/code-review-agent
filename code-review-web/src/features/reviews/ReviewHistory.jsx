  ...
  return (
    <>
      ...
      <button
        ...
        className="danger"
        disabled={isActive}
        title={isActive ? 'Cancel the review before deleting it' : 'Delete review'}
...
export default function ReviewHistory({ reviews, reviewPage, loadReviewPage, onDelete, onSelect }) {
  ...
  return (
    <section className="card">
      ...
      {reviews.length === 0 && (
        <p className="empty">No reviews yet. Start one from a public repository or paste code.</p>
      )}
      {reviews.map(review => {
          ...
          <div className="review-group" key={review.id}>
            <div className="review-row">
              <button
                ...
                onClick={() => (hasReruns ? toggleReruns(review) : onSelect(review))}
                aria-expanded={hasReruns ? isExpanded : undefined}
                aria-controls={hasReruns ? `reruns-${review.id}` : undefined}
              >
                <div>
                  <b>{sourceName(review)}</b>
                  <span className="muted">{reviewMetadata(review, 'Original review')}</span>
                </div>
              ...
              <div className="review-actions">
                {hasReruns && (
                  <span className="rerun-count">
                    {loadingRerunsId === review.id
                      ? 'Loading reruns…'
                      : `${review.rerunCount} rerun${review.rerunCount === 1 ? '' : 's'} ${isExpanded ? '▴' : '▾'}`}
                  </span>
                )}
                <button
                  type="button"
                  className="text-button review-view"
            ...
            {isExpanded && (
              <div className="rerun-list" id={`reruns-${review.id}`}>
                ...
                {!loadingRerunsId &&
                  reruns.map((rerun, index) => (
                    <div className="review-row rerun-row" key={rerun.id}>
                      <button type="button" className="review-open" onClick={() => onSelect(rerun)}>
                        <div>
                          <b>
                            Rerun {index + 1} · {sourceName(rerun)}
                          </b>
                          <span className="muted">
                            {reviewMetadata(rerun, `Rerun ${index + 1}`)}
                          </span>
                        </div>
                        <span className={`status ${rerun.status.toLowerCase()}`}>
                          {rerun.status.replaceAll('_', ' ')}
                        </span>
                      </button>
                      <div className="review-actions">
                        <DeleteAction
                          review={rerun}
                          onDelete={() => deleteRerun(rerun, review.id)}
                        />
                      </div>
                    </div>
                  ))}
                {!loadingRerunsId && reruns.length === 0 && (
                  <p className="muted">No reruns are available.</p>
                )}
              </div>
            )}
          </div>
      ...
      {reviewPage.total > PAGE_SIZE && (
        <div className="inline pagination">
          <button
            disabled={reviewPage.page === 0}
            onClick={() => loadReviewPage(reviewPage.page - 1)}
          >
            Previous
          </button>
          <span>Page {reviewPage.page + 1}</span>
          <button
            disabled={(reviewPage.page + 1) * PAGE_SIZE >= reviewPage.total}
            onClick={() => loadReviewPage(reviewPage.page + 1)}
          >
            Next
          </button>
