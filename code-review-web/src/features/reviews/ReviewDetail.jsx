...
export default function ReviewDetail({ review, rules, onRefresh, onDelete, onBack }) {
  ...
  return <div className="stack">
    ...
    <section className="card">
      ...
      <div className="filter-row"><label>Severity<select value={severity} onChange={e => { setSeverity(e.target.value); setPage(0); }}><option value="">All</option>{SEVERITIES.map(item => <option key={item}>{item}</option>)}</select></label>
        <label>Rule<select value={ruleId} onChange={e => { setRuleId(e.target.value); setPage(0); }}><option value="">All</option>{rules.map(rule => <option key={rule.id} value={rule.id}>{rule.name}</option>)}</select></label>
        <label>File<input value={file} onChange={e => { setFile(e.target.value); setPage(0); }} placeholder="Exact file path" /></label></div>
      {findings.items.length === 0 ? <p className="empty">No findings for these filters.</p> : findings.items.map(finding => <article className="finding" key={finding.id}>
        <div className="finding-head"><span className={`severity ${finding.severity.toLowerCase()}`}>{finding.severity}</span><h3>{finding.title}</h3><span className="muted">{finding.source}</span></div>
        <p className="location">{review.commitSha && finding.lineStart ? <a href={`${review.repositoryUrl}/blob/${review.commitSha}/${finding.filePath.split('/').map(encodeURIComponent).join('/')}#L${finding.lineStart}${finding.lineEnd !== finding.lineStart ? `-L${finding.lineEnd}` : ''}`} target="_blank" rel="noopener noreferrer">{finding.filePath}:{finding.lineStart}{finding.lineEnd !== finding.lineStart ? `–${finding.lineEnd}` : ''} ↗</a> : <>{finding.filePath}{finding.lineStart ? `:${finding.lineStart}${finding.lineEnd !== finding.lineStart ? `–${finding.lineEnd}` : ''}` : ''}</>}</p>
        <p>{finding.explanation}</p><pre className="evidence">{finding.evidence}</pre><p><b>Suggested fix:</b> {finding.suggestedFix}</p>
        <div className="finding-foot"><span className="muted">Rule: {rules.find(rule => rule.id === finding.ruleId)?.name || finding.ruleId} · v{finding.ruleVersion}</span>
          <label>Feedback<select value={finding.feedback || ''} onChange={e => feedback(finding.id, e.target.value)}><option value="">No feedback</option><option value="HELPFUL">Helpful</option><option value="IRRELEVANT">Irrelevant</option><option value="FALSE_POSITIVE">False positive</option></select></label></div>
      </article>)}
      {findings.total > PAGE_SIZE && <div className="inline pagination"><button disabled={page === 0} onClick={() => setPage(page - 1)}>Previous</button><span>Page {page + 1}</span><button disabled={(page + 1) * PAGE_SIZE >= findings.total} onClick={() => setPage(page + 1)}>Next</button></div>}
    </section>
  </div>;
}
