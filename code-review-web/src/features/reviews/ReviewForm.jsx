...
export default function ReviewForm({ sets, onCreated }) {
  ...
  const branches = [...new Set(repoInfo?.branches ?? [])]
    .filter(branch => branch && branch !== repoInfo?.defaultBranch);
  ...
  return <section className="card">
    ...
    <form onSubmit={submit} className="form-stack">
      {source === 'repository' ? <>
        <label>Public repository URL<input type="url" placeholder="https://github.com/owner/repo" value={repositoryUrl} onChange={e => changeRepositoryUrl(e.target.value)} required /></label>
        <div className="inline">
          <button type="button" onClick={inspect} disabled={busy || inspecting || !repositoryUrl}>{inspecting ? 'Inspecting…' : 'Inspect repository'}</button>
          {repoInfo && <span className="muted ref-name">Default branch: {repoInfo.defaultBranch}</span>}
        </div>
        <div className="ref-field">
          <label htmlFor="review-ref"><span>Branch, tag, or commit <span className="optional">(optional)</span></span></label>
          <select id="review-ref" className="ref-select" value={refChoice} onChange={e => setRefChoice(e.target.value)} aria-describedby="review-ref-help">
            <option value="">Use default branch{repoInfo?.defaultBranch ? ` (${repoInfo.defaultBranch})` : ''}</option>
          ...
          {refChoice === 'manual' && <label>Custom branch, tag, or commit<input value={manualRef} onChange={e => setManualRef(e.target.value)} maxLength={200} pattern="[A-Za-z0-9._/-]+" title="Use letters, numbers, periods, underscores, hyphens, or slashes" required placeholder="Branch, tag, or 40-character commit SHA" /></label>}
          <p id="review-ref-help" className="hint ref-help" role="status">
            {inspecting && 'Loading branches…'}
            {inspectStatus === 'idle' && 'Inspect the repository to list branches, or enter a ref manually.'}
            {inspectStatus === 'error' && 'Repository inspection failed. Correct the URL and retry, or enter a ref manually.'}
            {inspectStatus === 'success' && branches.length === 0 && 'No additional branches were returned. You can use the default branch or enter a ref manually.'}
            {inspectStatus === 'success' && branches.length > 0 && selectedBranch && <>Selected branch: <span className="ref-name">{selectedBranch}</span></>}
            {inspectStatus === 'success' && branches.length > 0 && refChoice === 'manual' && 'Enter the branch, tag, or commit to review.'}
            {inspectStatus === 'success' && branches.length > 0 && refChoice === '' && 'Choose a branch above, use the default branch, or enter another ref.'}
          </p>
        </div>
      </> : <>
      ...
      <button className="primary" disabled={busy || inspecting || !ruleSetId}>{busy ? 'Submitting…' : 'Start review'}</button>
    </form>
  </section>;
}
