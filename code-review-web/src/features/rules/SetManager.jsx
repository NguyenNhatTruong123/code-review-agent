...
export default function SetManager({ sets, rules, reload }) {
  ...
  async function save(event) {
    ...
    try { await api(`/rule-sets${editing ? `/${editing}` : ''}`, { method: editing ? 'PUT' : 'POST', body: form }); setEditing(null); setForm(EMPTY_RULE_SET); await reload(); }
  ...
  return <div className="stack"><section className="card"><h2>{editing ? 'Edit rule set' : 'Create rule set'}</h2><ErrorNotice message={error} clear={() => setError('')} />
    <form onSubmit={save} className="form-stack"><label>Name<input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required maxLength={120} /></label>
      ...
      <div className="inline"><button className="primary" disabled={form.ruleIds.length === 0}>{editing ? 'Save rule set' : 'Create rule set'}</button>{editing && <button type="button" onClick={() => { setEditing(null); setForm(EMPTY_RULE_SET); }}>Cancel</button>}</div>
    </form></section>
