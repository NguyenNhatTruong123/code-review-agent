...
public class ReviewWorker {
    ...
    public ReviewWorker(
            ...
            StaticReviewService staticReview,
            AiReviewService ai,
            ObjectMapper mapper) {
        this.reviews = reviews;
        this.findings = findings;
        this.github = github;
        this.staticReview = staticReview;
        this.ai = ai;
        this.mapper = mapper;
    }
    ...
    private void analyze(String id, List<SourceFile> files) throws Exception {
        ...
        List<RuleSnapshot> rules =
                Arrays.asList(mapper.readValue(review.ruleSnapshotJson, RuleSnapshot[].class));
        List<String> warnings = new ArrayList<>();
        if (review.warning != null && !review.warning.isBlank()) {
            warnings.add(review.warning);
        }
        ...
        for (SourceFile file : files) {
            ...
            if ("CANCELLED".equals(reviews.findById(id).orElseThrow().status)) {
                return;
            }
            List<RuleSnapshot> applicable =
                    rules.stream()
                            .filter(rule -> languageMatches(rule.languages(), file.language()))
                            .toList();
            List<StaticReviewService.Rule> literal =
                    applicable.stream()
                            .filter(rule -> rule.matchText() != null && !rule.matchText().isBlank())
                            .map(rule -> new StaticReviewService.Rule(rule.id(), rule.matchText()))
                            .toList();
            if (!literal.isEmpty()) {
                evaluated = true;
            }
                    ...
                    try {
                        ...
                        for (AiReviewService.Candidate candidate :
                            ...
                            if (rule == null || !validCandidate(candidate, file.code(), lines)) {
                                warnings.add("Discarded invalid AI finding for " + file.path());
                                continue;
                            }
                            saveFinding(
                                    id,
            ...
            review.scannedFiles = scanned;
            review.updatedAt = Instant.now();
            reviews.save(review);
        }
        review = reviews.findById(id).orElseThrow();
        if ("CANCELLED".equals(review.status)) {
            return;
        }
        review.warning =
                limit(String.join("; ", warnings.stream().distinct().limit(20).toList()), 2000);
        review.status =
                !evaluated
                        ? "FAILED"
                        : warnings.isEmpty() ? "COMPLETED" : "COMPLETED_WITH_WARNINGS";
        if (!evaluated) {
            review.error = "No rule could be evaluated for the selected source";
        }
        review.updatedAt = Instant.now();
        reviews.save(review);
    }
    ...
    static boolean validCandidate(AiReviewService.Candidate c, String code, int lines) {
        ...
        if (c.ruleId() == null
                ...
                || c.suggestedFix() == null
                || c.suggestedFix().isBlank()
                || c.evidence() == null
                || c.evidence().isBlank()
                || c.evidence().length() > 500
                || c.lineStart() == null
                || c.lineEnd() == null
                || c.lineStart() < 1
        ...
        return span.contains(c.evidence())
                && !looksSecret(c.evidence())
                && !looksSecret(c.title())
                && !looksSecret(c.explanation())
                && !looksSecret(c.suggestedFix());
    }
    ...
    private void saveFinding(
            ...
            String source,
            Set<String> dedupe) {
        if ("CANCELLED".equals(reviews.findById(reviewId).orElseThrow().status)) {
            return;
        }
        ...
        f.ruleVersion = rule.version();
        f.severity = rule.severity();
        f.title = limit(title, 200);
        f.explanation = limit(explanation, 4000);
        f.evidence = limit(evidence, 1000);
    ...
    private void fail(String id, Exception e) {
        ...
        if (review == null || "CANCELLED".equals(review.status)) {
            return;
        }
        ...
        review.error =
                e instanceof IOException
                        ? e.getMessage()
                        : "Review failed while loading or analyzing source code";
        review.updatedAt = Instant.now();
        reviews.save(review);
    }
}
