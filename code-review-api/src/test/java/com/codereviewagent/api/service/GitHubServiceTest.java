...
class GitHubServiceTest {
    private GitHubService service() {
        return new GitHubService(new ObjectMapper(), "", 100000, 10, 1000, 10000);
    }

    @Test
    void acceptsOnlyCanonicalPublicRepositoryUrl() {
        assertEquals(
                "https://github.com/acme/project",
                service().parse("https://github.com/acme/project.git").url());
        for (String url :
                new String[] {
                    "http://github.com/acme/project",
                    "https://github.com.evil.test/acme/project",
                    "https://user:pass@github.com/acme/project",
                    "https://github.com/acme/project/tree/main",
                    "https://github.com/acme/project?x=1",
                    "https://127.0.0.1/acme/project"
                }) {
    ...
    @Test
    void recognizesSupportedLanguages() {
    ...
    @Test
    void readsSourceAndReportsExcludedFiles() throws Exception {
        byte[] archive =
                zip(
                        new String[][] {
                            {"root/src/App.java", "class App {}"},
                            {"root/node_modules/a.js", "console.log(1)"},
    ...
    @Test
    void rejectsArchiveWithNoSupportedSource() throws Exception {
        try (ZipInputStream input =
                new ZipInputStream(
                        new ByteArrayInputStream(zip(new String[][] {{"root/a.png", "x"}})))) {
            assertThrows(IOException.class, () -> service().readArchive(input));
        }
    }

    @Test
    void rejectsOversizedSource() throws Exception {
        GitHubService limited = new GitHubService(new ObjectMapper(), "", 100000, 10, 5, 10000);
        try (ZipInputStream input =
                new ZipInputStream(
                        new ByteArrayInputStream(
                                zip(new String[][] {{"root/App.java", "class App {}"}})))) {
            assertThrows(IOException.class, () -> limited.readArchive(input));
        }
    }

    @Test
    void inspectsPublicRepositoryAndResolvesCommit() throws Exception {
        ...
        doAnswer(
                        inv -> {
                            ...
                            if (path.endsWith("/commits/main")) {
                                return json(200, "{\"sha\":\"" + "a".repeat(40) + "\"}");
                            }
        ...
        GitHubService service =
                new GitHubService(new ObjectMapper(), "", 100000, 10, 1000, 10000, client);
    ...
    @Test
    void rejectsPrivateRepositoryAndMissingRef() throws Exception {
        HttpClient client = mock(HttpClient.class);
        ...
        assertThrows(
                ...
                () -> service.inspect(new GitHubService.Repo("acme", "private")));
        assertThrows(
                ResponseStatusException.class,
                () -> service.resolveCommit(new GitHubService.Repo("acme", "private"), "../bad"));
    }

    @Test
    void downloadsArchiveOnlyFromCodeloadGitHub() throws Exception {
        HttpClient client = mock(HttpClient.class);
                                        ...
                                        .thenReturn(
                                                HttpHeaders.of(
                                                        Map.of(
                                                                "Location",
                                                                List.of(
                                                                        "https://codeload.github.com/acme/sample/zip/abc")),
                                                        (a, b) -> true));
                                return redirect;
                            }
        ...
        assertEquals(
                1,
                service.archive(new GitHubService.Repo("acme", "sample"), "a".repeat(40))
                        .files()
                        .size());
    }
