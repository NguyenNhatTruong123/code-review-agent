...
class RuleServiceTest {
    ...
    @BeforeEach void setup() {
        rules = mock(RuleRepository.class); sets = mock(RuleSetRepository.class);
    ...
    private static RuleInput input(String match) {
        return new RuleInput("No console", "Description", "STYLE", "LOW", "JAVASCRIPT", "Avoid console output", match, "Remove logging", true);
    }

    @Test void createsRuleAndIncrementsVersionOnEdit() {
    ...
    @Test void rejectsInvalidSeverityAndMultilineLiteral() {
        assertThrows(ResponseStatusException.class, () -> service.saveRule("owner", null,
                new RuleInput("X", "", "", "UNKNOWN", "ALL", "Check", "x", "Fix", true)));
        assertThrows(ResponseStatusException.class, () -> service.saveRule("owner", null, input("line\nbreak")));
    }

    @Test void deniesAccessToOtherUsersRule() {
    ...
    @Test void snapshotsOnlyEnabledOwnedRulesAndKeepsVersion() {
        RuleEntity enabled = new RuleEntity("one", "owner");
        enabled.name = "Check"; enabled.severity = "HIGH"; enabled.languages = "ALL";
        enabled.instruction = "Look for x"; enabled.matchText = "x"; enabled.enabled = true; enabled.version = 3;
        RuleEntity disabled = new RuleEntity("two", "owner"); disabled.enabled = false;
        RuleSetEntity set = new RuleSetEntity("set", "owner");
        set.enabled = true; set.ruleIdsJson = "[\"one\",\"two\"]";
    ...
    @Test void rejectsEmptyOrDisabledSet() {
        assertThrows(ResponseStatusException.class, () -> service.saveSet("owner", null, new SetInput("X", "", List.of(), true)));
        RuleSetEntity set = new RuleSetEntity("set", "owner"); set.enabled = false; set.ruleIdsJson = "[]";
        when(sets.findById("set")).thenReturn(Optional.of(set));
        assertThrows(ResponseStatusException.class, () -> service.snapshot("owner", "set"));
    }

    @Test void cannotDeleteRuleUsedBySet() {
        RuleEntity rule = new RuleEntity("rule", "owner");
        RuleSetEntity set = new RuleSetEntity("set", "owner"); set.ruleIdsJson = "[\"rule\"]";
    ...
    @Test void createsAndDeletesSetWithOwnedRule() {
        when(rules.findById("one")).thenReturn(Optional.of(new RuleEntity("one", "owner")));
        var created = service.saveSet("owner", null, new SetInput("Team set", "", List.of("one"), true));
    ...
    @Test void createsDefaultRulesAndSet() {
        java.util.Map<String, RuleEntity> saved = new java.util.HashMap<>();
        when(rules.save(any())).thenAnswer(inv -> {
            RuleEntity rule = inv.getArgument(0);
            saved.put(rule.id, rule);
            return rule;
        });
        when(rules.findById(any())).thenAnswer(inv -> Optional.ofNullable(saved.get(inv.getArgument(0))));
    ...
    @Test void rejectsRuleSetWithDuplicateOrForeignRule() {
        assertThrows(ResponseStatusException.class, () -> service.saveSet("owner", null,
                new SetInput("X", "", List.of("one", "one"), true)));
        when(rules.findById("other")).thenReturn(Optional.of(new RuleEntity("other", "someone-else")));
        assertThrows(ResponseStatusException.class, () -> service.saveSet("owner", null,
                new SetInput("X", "", List.of("other"), true)));
    }
}
