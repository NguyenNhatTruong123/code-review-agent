...
class ReviewServiceTest {
    ...
    @BeforeEach void setup() {
        reviews = mock(ReviewRepository.class); findings = mock(FindingRepository.class);
        rules = mock(RuleService.class); github = mock(GitHubService.class); worker = mock(ReviewWorker.class);
        ...
        when(rules.snapshot("owner", "set")).thenReturn(List.of(new RuleSnapshot("rule", 1, "Rule", "LOW", "ALL", "Check", "TODO", "Fix")));
        when(rules.ownedSet("set", "owner")).thenReturn(new RuleSetEntity("set", "owner"));
        when(rules.write(any())).thenReturn("[]");
    }

    @Test void createsPasteReviewWithDetectedLanguageAndNoStoredSource() {
        var result = service.createPaste("owner", new PasteInput("class X {}", "X.java", null, "set"), null);
        assertEquals("QUEUED", result.status());
        assertEquals("JAVA", result.language());
        verify(worker).paste(eq(result.id()), eq("class X {}"), eq("X.java"), eq("JAVA"));
        verify(reviews).save(argThat(r -> r.inputType.equals("PASTE") && r.ruleSnapshotJson.equals("[]")));
    }

    @Test void resolvesRepositoryToCommitBeforeDispatch() {
        var repo = new GitHubService.Repo("acme", "sample");
        when(github.parse("https://github.com/acme/sample")).thenReturn(repo);
        when(github.inspect(repo)).thenReturn(new GitHubService.RepoInfo(repo.url(), "main", List.of("main")));
        when(github.resolveCommit(repo, "main")).thenReturn("a".repeat(40));
        var result = service.createRepository("owner", new RepositoryInput(repo.url(), null, "set"), null);
    ...
    @Test void preventsIdempotencyKeyReuseWithDifferentInput() {
        ReviewEntity prior = new ReviewEntity("prior", "owner", "PASTE", "set");
        prior.inputHash = "different";
        when(reviews.findByOwnerIdAndIdempotencyKey("owner", "key"))
                .thenReturn(Optional.of(prior));
        assertThrows(ResponseStatusException.class, () -> service.createPaste("owner",
                new PasteInput("class X {}", "X.java", null, "set"), "key"));
        verify(worker, never()).paste(any(), any(), any(), any());
    }

    @Test void rejectsMissingLanguageAndOversizedPaste() {
        assertThrows(ResponseStatusException.class, () -> service.createPaste("owner", new PasteInput("x", "file.txt", null, "set"), null));
        assertThrows(ResponseStatusException.class, () -> service.createPaste("owner", new PasteInput("x".repeat(1001), "x.java", null, "set"), null));
    }

    @Test void neverShowsAnotherUsersReview() {
        when(reviews.findById("review")).thenReturn(Optional.of(new ReviewEntity("review", "other", "PASTE", "set")));
    ...
    @Test void validatesFeedbackAndFindingOwnership() {
        ...
        assertThrows(ResponseStatusException.class, () -> service.feedback("owner", "review", "finding", "HELPFUL"));
        finding.reviewId = "review";
        assertEquals("HELPFUL", service.feedback("owner", "review", "finding", "HELPFUL").feedback());
        assertThrows(ResponseStatusException.class, () -> service.feedback("owner", "review", "finding", "BAD"));
    }

    @Test void paginatesAndFiltersFindings() {
        ReviewEntity review = new ReviewEntity("review", "owner", "PASTE", "set");
        when(reviews.findById("review")).thenReturn(Optional.of(review));
        FindingEntity a = new FindingEntity("a", "review", "rule"); a.severity = "HIGH"; a.filePath = "A.java";
        FindingEntity b = new FindingEntity("b", "review", "rule"); b.severity = "LOW"; b.filePath = "B.java";
        when(findings.findByReviewIdOrderByFilePathAscLineStartAsc("review")).thenReturn(List.of(a, b));
        var page = service.findings("owner", "review", "HIGH", null, null, 0, 50);
        assertEquals(1, page.total());
        assertEquals("a", page.items().get(0).id());
        assertThrows(ResponseStatusException.class, () -> service.findings("owner", "review", null, null, null, -1, 50));
    }

    @Test void cancelOnlyActiveReview() {
        ReviewEntity review = new ReviewEntity("review", "owner", "PASTE", "set");
        when(reviews.findById("review")).thenReturn(Optional.of(review));
    ...
    @Test void marksInterruptedJobsFailedAfterRestart() {
        ...
        when(reviews.findByStatusIn(List.of("QUEUED", "RUNNING"))).thenReturn(List.of(queued, running));
    ...
    @Test void listsOnlyOnePageOfCurrentUsersReviews() {
        ReviewEntity review = new ReviewEntity("review", "owner", "PASTE", "set");
