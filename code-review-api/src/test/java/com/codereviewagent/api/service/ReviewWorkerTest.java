...
class ReviewWorkerTest {
    @Test
    void staticReviewCreatesFindingAndCompletes() throws Exception {
    ...
    @Test
    void semanticOnlyWithoutProviderFailsHonestly() throws Exception {
        ReviewRepository reviews = mock(ReviewRepository.class);
        ReviewEntity review = new ReviewEntity("review", "owner", "PASTE", "set");
        review.ruleSnapshotJson =
                new ObjectMapper()
    ...
    @Test
    void acceptsValidatedSemanticFinding() throws Exception {
        ReviewRepository reviews = mock(ReviewRepository.class);
        FindingRepository findings = mock(FindingRepository.class);
        AiReviewService ai = mock(AiReviewService.class);
        ReviewEntity review = new ReviewEntity("review", "owner", "PASTE", "set");
        ...
        when(reviews.save(any())).thenAnswer(inv -> inv.getArgument(0));
        when(ai.available()).thenReturn(true);
        when(ai.review(any(), any(), any(), any()))
                .thenReturn(
                        List.of(
                                new AiReviewService.Candidate(
                                        "rule",
                                        "Unfinished",
                                        "TODO marker",
                                        "TODO",
    ...
    @Test
    void failsWhenGitHubArchiveCannotBeLoaded() throws Exception {
        ReviewRepository reviews = mock(ReviewRepository.class);
        GitHubService github = mock(GitHubService.class);
        ...
        when(github.archive(repo, "a".repeat(40)))
                .thenThrow(new IOException("Repository exceeds source file limit"));
        ReviewWorker worker =
                new ReviewWorker(
                        reviews,
                        mock(FindingRepository.class),
    ...
    @Test
    void doesNotStartCancelledReview() {
        ReviewRepository reviews = mock(ReviewRepository.class);
        ReviewEntity review = new ReviewEntity("review", "owner", "PASTE", "set");
    ...
    @Test
    void validatesAiEvidenceAndLineRange() {
        var valid =
                new AiReviewService.Candidate("rule", "Title", "Explanation", "TODO", "Fix", 2, 2);
        assertTrue(ReviewWorker.validCandidate(valid, "first\nTODO", 2));
        assertFalse(ReviewWorker.validCandidate(valid, "TODO\nfirst", 2));
        assertFalse(
                ReviewWorker.validCandidate(
                        new AiReviewService.Candidate(
                                "rule", "Title", "Explanation", "TODO", "Fix", 3, 3),
                        "first\nTODO",
                        2));
        assertFalse(
                ReviewWorker.validCandidate(
                        new AiReviewService.Candidate(
                                "rule", "", "Explanation", "TODO", "Fix", 2, 2),
                        "first\nTODO",
                        2));
        assertFalse(
                ReviewWorker.validCandidate(
    ...
    @Test
    void matchesAllOrNamedLanguage() {
